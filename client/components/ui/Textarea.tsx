...
export const Textarea: FC<Props> = ({ resize, label, onChange, value, className, ...props }) => {
		...
		<div className={cn(className, 'relative h-fit w-fit')}>
			<label
				className={cn(
					...
					{
						'top-[22px] translate-y-[-50%] left-4 text-[inherit]/[inherit]': !isFocused && !value,
						'top-0 translate-y-[-50%] left-2 text-[14px]/[14px]': isFocused || value
					}
				)}
			>
				{label}
